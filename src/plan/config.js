...
import { LEGACY_EXIT_PLAN_MODE_TOOL } from "./constants.js";
...
function parseProfile(value, source, key, warnings) {
  ...
  if ((profile.provider && !profile.model) || (!profile.provider && profile.model)) {
    warnings.push(
      `${source}: "${key}.provider" and "${key}.model" must be configured together; both were ignored.`,
    );
...
function parseTools(value, source, warnings) {
  ...
  value.forEach((item, index) => {
    ...
    if (name === LEGACY_EXIT_PLAN_MODE_TOOL) {
      warnings.push(
...
function readConfigFile(filePath, warnings) {
  ...
  try {
    ...
    if (parsed.normal === undefined && parsed.execution !== undefined) {
      warnings.push(
        `${filePath}: migrated legacy "execution" model/thinking profile to "normal".`,
      );
    }
    return {
    ...
    warnings.push(
      `${filePath}: unable to read configuration: ${
          ...
          tools: [
            ...new Set(
              config.tools
                .map((name) => name.trim())
                .filter(
                  (name) => name && name !== LEGACY_EXIT_PLAN_MODE_TOOL,
                ),
            ),
          ],
        }
